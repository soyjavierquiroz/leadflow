...
echo "🧽 Ejecutando hard reset local..."
./hard-reset-build.sh

echo "📦 Construyendo API..."
docker build --pull --no-cache -t leadflow-api:$TAG -t leadflow-api:latest -f apps/api/Dockerfile .
...
docker build --pull --no-cache -t leadflow-web:$TAG -t leadflow-web:latest -f apps/web/Dockerfile .
