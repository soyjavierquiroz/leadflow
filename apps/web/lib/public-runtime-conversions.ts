...
import { leadCaptureConversionEventName } from "@/lib/public-runtime-conversion-event";
...
export const emitLeadCaptureConversionEvent = ({
  ...
  if (!hasMetaPixel && !hasTikTokPixel) {
    return;
  }

  const eventId = payload.submissionEventId ?? undefined;
  const normalizedBlockType = normalizeBlockType(block.type);
  const nextResolvedStepPath = nextStepPath ?? response.nextStep?.path ?? null;
  const conversionMetadata = {
    ...
    next_step_path: nextResolvedStepPath,
  ...
  if (hasMetaPixel && typeof fbq !== "function") {
  ...
  } else if (hasMetaPixel && typeof fbq === "function") {
    fbq(
      "track",
  ...
  if (hasTikTokPixel && typeof ttqTrack === "function") {
